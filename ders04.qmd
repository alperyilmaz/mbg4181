...
data_tsv_url <- "https://raw.githubusercontent.com/alperyilmaz/data/refs/heads/main/states.tsv"
...
## Tarih İşlemleri ile Veri Analizi

```{webr}
# Tarih verileri üzerinde işlemler
tarih_data |>
  mutate(
    yaş = as.numeric(difftime(Sys.Date(), doğum_tarihi, units = "days") / 365.25),
    kayıt_ayı = month(kayıt_zamanı, label = TRUE),
    kayıt_günü = wday(kayıt_zamanı, label = TRUE),
    kayıt_saati = hour(kayıt_zamanı)
  )
```
